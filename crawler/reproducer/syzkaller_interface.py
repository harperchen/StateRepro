...
class SyzkallerInterface(AnalysisModule):
    ...
    def build_syzkaller(self, arch, component=None):
        ...
        if component == None:
            ...
            p = Popen(["make", "TARGETARCH={}".format(arch), "TARGETVMARCH=amd64"], cwd=self.syzkaller_path, env=my_env,
                      stdout=PIPE, stderr=STDOUT)
        else:
            self.logger.info("make TARGETARCH={} TARGETVMARCH=amd64 {}".format(arch, component))
            p = Popen(["make", "TARGETARCH={}".format(arch), "TARGETVMARCH=amd64", component], cwd=self.syzkaller_path,
                      env=my_env, stdout=PIPE, stderr=STDOUT)
        with p.stdout:
            log_anything(p.stdout, self.logger, self.debug)
        exitcode = p.wait()
        if not os.path.exists(os.path.join(self.syzkaller_path, "workdir")):
    ...
    def pull_cfg_for_cur_case(self, linux):
        ...
        syz_config = cfg_template.format(self.syzkaller_path, linux_path, image_path, "", 4,
                                         self.cfg.get_kernel_by_name('upstream').ssh_port)
