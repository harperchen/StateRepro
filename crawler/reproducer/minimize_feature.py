...
from utils import *
from strings import *
from syzkaller_interface import SyzkallerInterface
...
class SyzFeatureMinimize(AnalysisModule):
    ...
    def generate_new_PoC(self, features):
        ...
        local_command(command='chmod +x syz-prog2c && {} > {}/PoC_repeat.c'.format(prog2c_cmd, self.path_case_plugin),
                      logger=self.logger, \
                      shell=True, cwd=self.syz.path_case_plugin)
        if self._file_is_empty(os.path.join(self.path_case_plugin, "PoC_repeat.c")):
            return False
        shutil.copyfile(os.path.join(self.path_case_plugin, "PoC_repeat.c"),
        ...
        local_command(
            command='chmod +x syz-prog2c && {} > {}/PoC_no_repeat.c'.format(prog2c_cmd, self.path_case_plugin),
            logger=self.logger, \
            shell=True, cwd=self.syz.path_case_plugin)
        if self._file_is_empty(os.path.join(self.path_case_plugin, "PoC_no_repeat.c")):
            return False
        shutil.copyfile(os.path.join(self.path_case_plugin, "PoC_no_repeat.c"),
    ...
    def _test_feature(self, rule_out_feature, essential_features: list, root, test_c_prog=False, repeat=False,
        ...
        _, triggered, _ = distro.repro.reproduce(func=self._capture_crash,
                                                 ...
                                                 root=root, work_dir=self.path_case_plugin, c_hash=self.case_hash)
    ...
    def _capture_crash(self, qemu: VM, root: bool, features: list, test_c_prog: bool, repeat: bool, sandbox: str):
        ...
        if test_c_prog:
            ...
            local_command(command='chmod +x syz-prog2c && {} > {}/poc.c'.format(prog2c_cmd, self.path_case_plugin),
                          logger=self.logger, \
                          shell=True, cwd=self.syz.path_case_plugin)
