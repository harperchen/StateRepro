...
from errors import *
from config import *
from reproducer.reproducer import *

reserve_port = 7


class Case:
    def __init__(self, args, index, proj_dir, case_hash):
        self.cfg: Config = self.parse_config(args.config)
        self.index = index
        self.case_hash = case_hash[:7]
        self._success = False
        self.path_project = proj_dir
    ...
    def read_case(self):
        work_path = os.getcwd()
        cases_json_path = os.path.join(work_path, "{}/{}/crash_consoles.json".format(self.path_project, self.case_hash))
