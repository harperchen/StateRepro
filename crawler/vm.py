...
class VM(VMState):
    ...
    def __init__(self, linux, kernel, port, image, hash_tag, key, vmlinux=None, tag='', arch='amd64', work_path='/tmp/',
        ...
        if kernel.type == self.ANDROID:
            self._vm_instance = AEmuInstance(tag=tag, work_path=work_path, log_name=log_name, log_suffix=log_suffix,
                                             logger=logger, hash_tag=hash_tag, debug=debug)
            self._vm_instance.setup(linux=linux, kernel=kernel, port=port, image=image, mem=mem, cpu=cpu, key=key,
